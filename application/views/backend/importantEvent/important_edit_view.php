...
<form action="<? echo $url['action']?>" method="post"  id="update_form" class="contentEditForm" >
		...
		<table  border="0" cellspacing="0" cellpadding="0" id='dataTable'>
			...
			<tr>
				<td class="left"><span class="require">* </span>日期</td>
				<td> <? $filed='date'?>
				<input  name="<? echo $filed?>" type="text" class='middle'  value="<? echo tryGetArrayValue($filed,$edit_data,"")?>"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
				<?php echo form_error($filed); ?></td>
			</tr>
			<tr>
