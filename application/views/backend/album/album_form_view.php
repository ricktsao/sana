...
<form action="<? echo bUrl("updateAlbum")?>" method="post"  id="update_form" enctype="multipart/form-data" class="contentEditForm">
	<div id="option_bar">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td >
					<button type="button" class='btn back' onclick="jUrl('<?php echo bUrl("albumList")?>')"><?php echo $this->lang->line('common_return'); ?></button>
				</td>
			</tr>
		</table>
	</div>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" id='dataTable'>
		...
		<tr>
			...
			<td>
				<select  name="album_category_sn">		
				<?php foreach ($cat_list as $key => $item){?>
					<option value="<?php echo $item["sn"];?>"  <?php echo tryGetData('album_category_sn',$edit_data)==$item["sn"]?"selected":"" ?>><?php echo $item["title"];?></option>
				<?php } ?>					
				</select>
		...
		<tr>
			<td colspan="2" class='center'>
				<button type="button" class='btn back' onclick="jUrl('<?php echo bUrl("albumList")?>')"><?php echo $this->lang->line('common_cancel'); ?></button>        	
