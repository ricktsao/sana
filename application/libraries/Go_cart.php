...
*/
	...
	private function _apply_coupon_to_product($cartkey, $coupon_code) 
	{
		...
		$price = (int) $this->_cart_contents['items'][$cartkey]['price'];
...
//			$str = ' - ($subtotal * ('. $reduction_ammount .' /100))';
			...
			$reduction_amount	= 100 - (int) $this->_cart_contents['coupon_list'][$coupon_code]['reduction_amount'];
			$discount			= ($price * ($reduction_amount/100));
			$discount_amount	= abs($price-$discount);
		} 
		else
		{
			$discount_amount = (int) $this->_cart_contents['coupon_list'][$coupon_code]['reduction_amount']; 
...
//var_dump($items);die;
	...
	function set_shipping($method, $price)
	{
		...
		$this->_cart_contents['shipping'] = array('method'=>$method, 'price'=> (int) $price);
	...
	function save_order($data, $plus_arr=array()) {
		...
		$order_id = $this->CI->order_model->save_order($data, $contents, $plus_arr);
	...
	function save_order_xxx($data) {
		...
		// Is this a non shippable order?  檢查訂購商品中是否有無法運送之商品
		$none_shippable = true;
		foreach ($this->_cart_contents['items'] as $item)
		{
			if($item['shippable']==1)
		...
		$save['ship_name']			= $ship['name'];
		$save['ship_email']			= $ship['email'];
		$save['ship_phone']			= $ship['phone'];
		$save['ship_address']		= $ship['address'];
	...
	function total()
	{
		return (int) $this->_cart_contents['cart_total'];
	}

	function subtotal()
	{
		return (int) $this->_cart_contents['cart_subtotal'];
	}
