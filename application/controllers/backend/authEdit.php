...
class AuthEdit extends Backend_Controller 
{
	...
	public function index()
	{
		...
		$admin_info = $this->main_model->listData("sys_admin", NULL ,"sn =".$admin_sn);
		$data['edit_data']=array();
		if(count($admin_info["data"])==0)
		{			
			redirect(getFrontendControllerUrl("index"));	
		}else{
	...
	public function update()
	{
        ...
        {			
        	$arr_data["pw"] = prepPassword($edit_data["password"]);
      	 	$arr_return=$this->main_model->updateDB( "sys_admin" , $arr_data, "sn =".$admin_sn );
